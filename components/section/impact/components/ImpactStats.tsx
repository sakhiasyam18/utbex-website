...
export function ImpactStats() {
    return (
        <motion.div
            ...
            className="grid grid-cols-1 sm:grid-cols-3 gap-10 md:gap-8 lg:gap-16 border-b border-neutral-200/60 pb-16 mb-16"
        >
            {impactContent.stats.map((stat) => (
                <motion.div key={stat.id} variants={fadeInUp} className="flex flex-col">
                    ...
                    />
                    <span className="text-sm md:text-base font-semibold text-neutral-500 uppercase tracking-wider">
                        {stat.label}
                    </span>
                </motion.div>
            ))}
