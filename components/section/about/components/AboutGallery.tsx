...
function Caption({ annotation, align = "left" }: { annotation?: GalleryImageAnnotation, align?: "left" | "right" }) {
  ...
  return (
    <div className={`absolute top-3 sm:top-4 ${align === "right" ? "right-3 sm:right-4 text-right" : "left-3 sm:left-4 text-left"} z-20 flex flex-col gap-0 text-[9px] sm:text-[10px] uppercase tracking-wider bg-white/85 backdrop-blur-md px-3 py-1.5 rounded-lg shadow-sm border border-black/5`}>
      <span className="font-semibold text-neutral-900">{annotation.title}</span>
      <span className="text-neutral-600">{annotation.location} — {annotation.detail}</span>
    </div>
  );
}
